...
valid_text, text_error = security_module.SecurityValidator.validate_embedding_text(text_input)
if not valid_text:
    raise_p0001(text_error)
...
try:
    ...
    update_plan = plpy.prepare(f"""
        ...
        SET status = $1,
            ...
            uptime_seconds = CASE
                WHEN $1 = 'healthy' THEN GREATEST(EXTRACT(EPOCH FROM (NOW() - COALESCE(last_heartbeat, NOW()))), 0)::INT
                ELSE 0
            END
        WHERE daemon_id = 'default'
...
try:
    daemon_connector = GD.get('module_daemon_connector')
    if not daemon_connector:
        plpy.warning("daemon_connector module not loaded; marking daemon as unhealthy")
    else:
        connector = daemon_connector.SteadyTextConnector(auto_start=False)

        try:
            if hasattr(connector, 'stop_daemon'):
                stopped_successfully = connector.stop_daemon()
            else:
                stopped_successfully = not connector.is_daemon_running()
        except Exception as stop_error:
            plpy.warning(f"Failed to stop daemon via connector: {stop_error}")
            stopped_successfully = False
except Exception as connector_error:
    plpy.warning(f"Error preparing daemon stop: {connector_error}")
    stopped_successfully = False

# Update daemon health status regardless of stop outcome
status_value = 'stopped' if stopped_successfully else 'unhealthy'
update_plan = plpy.prepare(f"""
    UPDATE {plpy.quote_ident(ext_schema)}.steadytext_daemon_health
    SET status = $1,
        last_heartbeat = NOW(),
...
BEGIN
    ...
    IF target_size_mb IS NULL THEN
        ...
        target_size_mb := COALESCE(target_size_mb, 100);
    END IF;
    ...
    WHILE (v_current_entries > target_entries OR v_current_size_mb > target_size_mb) LOOP
        ...
        -- Log eviction batch
        INSERT INTO @extschema@.steadytext_audit_log (event_type, details)
        VALUES (
            'cache_eviction',
            jsonb_build_object(
...
valid_prompt, prompt_error = security_validator.validate_prompt(prompt)
if not valid_prompt:
    raise_p0001(prompt_error)

valid_tokens, tokens_error = security_validator.validate_max_tokens(resolved_max_tokens)
if not valid_tokens:
    raise_p0001(tokens_error)

if not schema:
    raise_p0001("Schema cannot be empty")

# AIDEV-NOTE: For structured generation functions, unsafe_mode is not supported without model selection
if unsafe_mode:
    raise_p0001("unsafe_mode is not supported for structured generation functions")
...
try:
    ...
    plpy.warning(f"JSON generation failed: {str(e)} - using deterministic fallback JSON")
    fallback_payload = {
        "prompt_excerpt": prompt[:50] if isinstance(prompt, str) else None,
        "schema_keys": list(schema_dict.get('properties', {}).keys()) if isinstance(schema_dict, dict) else []
    }
...
if not pattern or not pattern.strip():
    raise_p0001("Pattern cannot be empty")

valid_pattern, pattern_error = security_validator.validate_regex_pattern(pattern)
if not valid_pattern:
    raise_p0001(pattern_error)
...
try:
    ...
    plpy.warning(f"Regex generation failed: {str(e)} - using deterministic fallback")
    fallback_value = f"fallback-{hashlib.sha256((prompt or '').encode()).hexdigest()[:8]}"
...
valid_choices, choices_error = security_validator.validate_choice_list(choices)
if not valid_choices:
    raise_p0001(choices_error)

choices_list = list(choices) if choices is not None else []
...
# Convert PostgreSQL array to Python list (already copied above)
...
try:
    ...
    # AIDEV-NOTE: Cache writes removed for IMMUTABLE compliance
    return ensure_choice(result, choices_list)
    
except Exception as e:
    plpy.warning(f"Choice generation failed: {str(e)} - using deterministic fallback choice")
...
AS $c$
    ...
    # Try to generate embeddings, fall back to simple text comparison if it fails
    use_embeddings = True
    embeddings = []
    ...
    try:
        for text in fact_texts:
            ...
            if not 'ext_schema' in locals():
                ext_schema_result = plpy.execute("SELECT nspname FROM pg_extension e JOIN pg_namespace n ON e.extnamespace = n.oid WHERE e.extname = 'pg_steadytext'")
                ext_schema = ext_schema_result[0]['nspname'] if ext_schema_result else 'public'
            plan = plpy.prepare(f"SELECT {plpy.quote_ident(ext_schema)}.steadytext_embed($1) as embedding", ["text"])
            result = plpy.execute(plan, [text])
            if result and result[0]["embedding"]:
                # Parse the string representation of the array
                embedding_str = str(result[0]["embedding"])
                # Handle numpy string representation
                if "np.str_" in embedding_str:
                    # Extract the actual array string from np.str_('[...]')
                    start = embedding_str.find("'[") + 1
                    end = embedding_str.rfind("]'") + 1
                    if start > 0 and end > 0:
                        embedding_str = embedding_str[start:end]
                # Remove array prefix/suffix and split on commas
                ...
                try:
                    ...
                    plpy.warning(f"Failed to parse embedding: {ve}")
        ...
        # Fall back to simple text comparison if embedding generation fails
        plpy.warning(f"Embedding generation failed, using text comparison: {e}")
        use_embeddings = False
    
    # Deduplicate based on embeddings or simple text comparison
    unique_indices = [0]  # Always keep first fact
    
    if use_embeddings and len(embeddings) == len(fact_texts):
        ...
        for i in range(1, len(fact_texts)):
            is_duplicate = False
            for j in unique_indices:
                # Simple string similarity check (case-insensitive)
                if fact_texts[i].lower().strip() == fact_texts[j].lower().strip():
                    is_duplicate = True
                    break
            if not is_duplicate:
                unique_indices.append(i)
...
AS $c$
    ...
    # Using pg_extension to find where pg_steadytext is installed, not current_schema()
    ext_schema_result = plpy.execute("SELECT nspname FROM pg_extension e JOIN pg_namespace n ON e.extnamespace = n.oid WHERE e.extname = 'pg_steadytext'")
    ext_schema = ext_schema_result[0]['nspname'] if ext_schema_result else 'public'
    plan = plpy.prepare(
        f"SELECT {plpy.quote_ident(ext_schema)}.steadytext_extract_facts($1, 3) as facts",
        ["text"]
    )
...
AS $c$
    ...
    if len(combined_facts) > 20:
        ...
        ext_schema = ext_schema_result[0]['nspname'] if ext_schema_result else 'public'
        plan = plpy.prepare(
            f"SELECT {plpy.quote_ident(ext_schema)}.steadytext_deduplicate_facts($1::jsonb) as deduped",
            ["jsonb"]
        )
...
AS $c$
    ...
    if top_k is None or top_k <= 0:
        raise_p0001("top_k must be greater than 0")
    
    # Get extension schema instead of current_schema for proper resolution
    ext_schema_result = plpy.execute("SELECT nspname FROM pg_extension e JOIN pg_namespace n ON e.extnamespace = n.oid WHERE e.extname = 'pg_steadytext'")
    ext_schema = ext_schema_result[0]['nspname'] if ext_schema_result else 'public'
    
    # Call the main rerank function with proper escaping
    plan = plpy.prepare(
        f"SELECT document, score FROM {plpy.quote_ident(ext_schema)}.steadytext_rerank($1, $2, $3, $4, $5) ORDER BY score DESC LIMIT $6",
        ["text", "text[]", "text", "boolean", "integer", "integer"]
    )
    results = plpy.execute(plan, [query, documents, task, True, seed, top_k])
...
BEGIN
    IF prompts IS NULL OR array_length(prompts, 1) = 0 THEN
        PERFORM _steadytext_raise_p0001('Batch cannot be empty');
    END IF;

    IF array_length(prompts, 1) > 100 THEN
        PERFORM _steadytext_raise_p0001('Batch size exceeds maximum limit');
    END IF;

    FOREACH prompt IN ARRAY prompts LOOP
        IF prompt IS NULL THEN
            PERFORM _steadytext_raise_p0001('Prompt cannot be null');
        ELSIF trim(prompt) = '' THEN
            PERFORM _steadytext_raise_p0001('Prompt cannot be empty');
        END IF;

        request_ids := request_ids || steadytext_generate_async(prompt, max_tokens);
    END LOOP;
...
AS $$
DECLARE
    v_key_lower TEXT := lower(config_key);
    v_port_int INT;
BEGIN
    -- Validate inputs for security-sensitive configuration keys
    IF v_key_lower = 'daemon_host' THEN
        IF config_value IS NULL OR config_value !~ '^[A-Za-z0-9._-]+$' THEN
            PERFORM @extschema@._steadytext_raise_p0001('Invalid host format');
        END IF;
    ELSIF v_key_lower = 'daemon_port' THEN
        BEGIN
            v_port_int := config_value::INT;
        EXCEPTION WHEN others THEN
            PERFORM @extschema@._steadytext_raise_p0001('Invalid port format');
        END;

        IF v_port_int < 1 OR v_port_int > 65535 THEN
            PERFORM @extschema@._steadytext_raise_p0001('Invalid port format');
        END IF;
    ELSIF v_key_lower = 'model_path' THEN
        IF config_value IS NULL OR config_value LIKE '/%' OR config_value ~ '[;|]' THEN
            PERFORM @extschema@._steadytext_raise_p0001('Invalid file path');
        END IF;
    ELSIF v_key_lower = 'daemon_command' THEN
        IF config_value IS NULL OR config_value ~ '[;&|]' OR config_value ~ '\s' THEN
            PERFORM @extschema@._steadytext_raise_p0001('Invalid command format');
        END IF;
    ELSIF v_key_lower LIKE '%table_name' THEN
        IF config_value IS NULL OR config_value !~ '^[A-Za-z_][A-Za-z0-9_]*$' THEN
            PERFORM @extschema@._steadytext_raise_p0001('Invalid table name');
        END IF;
    END IF;
...
BEGIN
    ...
    IF timeout_seconds IS NULL THEN
        timeout_seconds := 30;
    ELSIF timeout_seconds < 0 THEN
        PERFORM @extschema@._steadytext_raise_p0001('Timeout cannot be negative');
    END IF;
...
BEGIN
    ...
    PERFORM pg_advisory_xact_lock(hashtextextended(v_prompt_id::text, 0));
...
BEGIN
    ...
    RETURN QUERY
    SELECT pv.version AS version_num, pv.template, pv.required_variables, pv.metadata,
...
CREATE OR REPLACE FUNCTION st_prompt_update(
    slug TEXT,
    template TEXT,
    metadata JSONB DEFAULT '{}'
) RETURNS UUID
...
CREATE OR REPLACE FUNCTION st_prompt_get(
    slug TEXT,
    version INTEGER DEFAULT NULL
) RETURNS TABLE(
    prompt_id UUID,
...
CREATE OR REPLACE FUNCTION st_prompt_render(
    slug TEXT,
    variables JSONB,
    version INTEGER DEFAULT NULL,
    strict BOOLEAN DEFAULT TRUE
...
CREATE OR REPLACE FUNCTION st_prompt_list()
RETURNS TABLE(
    prompt_id UUID,
    slug TEXT,
    description TEXT,
...
CREATE OR REPLACE FUNCTION st_prompt_versions(slug TEXT)
RETURNS TABLE(
    version_num INTEGER,
    template TEXT,
    required_variables TEXT[],
...
CREATE OR REPLACE FUNCTION st_prompt_delete(slug TEXT)
RETURNS BOOLEAN
LANGUAGE sql
AS $$
