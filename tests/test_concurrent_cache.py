...
class TestConcurrentCache:
    ...
    def test_concurrent_read_write_threads(self, temp_cache_dir):
        ...
        for i in range(100, 150):
            key = f"writer_key_{i}"
            value = cache.get(key)
            assert value == f"writer_value_{i}", f"Missing or incorrect writer data for {key}"
    ...
    def test_concurrent_processes(self, temp_cache_dir):
        ...
        with ProcessPoolExecutor(max_workers=num_processes) as executor:
            futures = [executor.submit(self._process_worker, args) for args in args_list]
    ...
    def test_eviction_under_concurrent_load(self, temp_cache_dir):
        ...
        assert remaining_entries < 200, f"Expected eviction, but {remaining_entries} entries remain"
        assert remaining_entries > 50, f"Too many entries evicted: only {remaining_entries} remain"
    
    def test_database_locking_behavior(self, temp_cache_dir):
        """Test that database locking works correctly under high contention."""
        cache = SQLiteDiskBackedFrecencyCache(
            capacity=100,
            cache_name="locking_test",
            cache_dir=temp_cache_dir
        )
        ...
        assert operation_count == 8 * 80, f"Expected 640 operations, got {operation_count}"
    ...
    def test_cache_persistence_across_processes(self, temp_cache_dir):
        ...
        for i, (key, value) in enumerate(read_results):
            ...
            assert value == expected_value, f"Value mismatch for {key}: {value} != {expected_value}"
    ...
    def test_stress_test_mixed_operations(self, temp_cache_dir):
        ...
        assert operations_completed == 6 * 100, f"Expected 600 operations, got {operations_completed}"
        
        # Verify cache is still functional
        stats = cache.get_stats()
        assert stats["entry_count"] > 0, "Cache should not be empty after stress test"
        assert stats["total_size_bytes"] <= cache.max_size_bytes, "Cache size should be within limits"