...
from steadytext.cache_manager import get_cache_manager, get_generation_cache, get_embedding_cache
...
class TestDaemonCacheIntegration:
    ...
    def test_generation_cache_consistency_direct_vs_daemon(self):
        ...
        # Step 2: Start a daemon server in thread for testing
        server = DaemonServer(host="localhost", port=5556, preload_models=False)
        ...
        try:
            ...
            os.environ["STEADYTEXT_DAEMON_PORT"] = "5556"
            
            client = DaemonClient(host="localhost", port=5556)
            connected = client.connect()
            assert connected, "Failed to connect to test daemon"
            
            daemon_result = client.generate(test_prompt)
            assert isinstance(daemon_result, str)
            assert daemon_result == direct_result, "Daemon result doesn't match cached direct result"
    ...
    def test_streaming_cache_consistency(self):
        ...
        direct_stream_result = "".join(direct_stream_tokens)
        # Note: Direct streaming might not use cache, but daemon should
        
        # Step 3: Start daemon and test streaming cache usage
        server = DaemonServer(host="localhost", port=5557, preload_models=False)
        server_thread = threading.Thread(target=server.run, daemon=True)
        server_thread.start()
        time.sleep(2)
        ...
        try:
            if "STEADYTEXT_DISABLE_DAEMON" in os.environ:
                del os.environ["STEADYTEXT_DISABLE_DAEMON"]
            os.environ["STEADYTEXT_DAEMON_HOST"] = "localhost"
            os.environ["STEADYTEXT_DAEMON_PORT"] = "5557"
            
            client = DaemonClient(host="localhost", port=5557)
            ...
            assert daemon_stream_result.strip() == direct_result.strip()
    ...
    def test_embedding_cache_consistency(self):
        ...
        server = DaemonServer(host="localhost", port=5558, preload_models=False)
        server_thread = threading.Thread(target=server.run, daemon=True)
        server_thread.start()
        time.sleep(2)
        ...
        try:
            if "STEADYTEXT_DISABLE_DAEMON" in os.environ:
                del os.environ["STEADYTEXT_DISABLE_DAEMON"]
            os.environ["STEADYTEXT_DAEMON_HOST"] = "localhost"
            os.environ["STEADYTEXT_DAEMON_PORT"] = "5558"
            
            client = DaemonClient(host="localhost", port=5558)
            assert client.connect(), "Failed to connect to daemon"
    ...
    def test_cache_miss_and_population_via_daemon(self):
        ...
        # Start daemon
        server = DaemonServer(host="localhost", port=5559, preload_models=False)
        server_thread = threading.Thread(target=server.run, daemon=True)
        server_thread.start()
        time.sleep(2)
        ...
        try:
            if "STEADYTEXT_DISABLE_DAEMON" in os.environ:
                del os.environ["STEADYTEXT_DISABLE_DAEMON"]
            os.environ["STEADYTEXT_DAEMON_HOST"] = "localhost"
            os.environ["STEADYTEXT_DAEMON_PORT"] = "5559"
            
            client = DaemonClient(host="localhost", port=5559)
            assert client.connect(), "Failed to connect to daemon"
    ...
    def test_custom_eos_string_cache_key(self):
        ...
        # Start daemon
        server = DaemonServer(host="localhost", port=5560, preload_models=False)
        server_thread = threading.Thread(target=server.run, daemon=True)
        server_thread.start()
        time.sleep(2)
        ...
        try:
            if "STEADYTEXT_DISABLE_DAEMON" in os.environ:
                del os.environ["STEADYTEXT_DISABLE_DAEMON"]
            os.environ["STEADYTEXT_DAEMON_HOST"] = "localhost"
            os.environ["STEADYTEXT_DAEMON_PORT"] = "5560"
            
            client = DaemonClient(host="localhost", port=5560)
            assert client.connect(), "Failed to connect to daemon"
    ...
    def test_no_cache_for_logprobs_requests(self):
        ...
        # Start daemon
        server = DaemonServer(host="localhost", port=5561, preload_models=False)
        server_thread = threading.Thread(target=server.run, daemon=True)
        server_thread.start()
        time.sleep(2)
        ...
        try:
            if "STEADYTEXT_DISABLE_DAEMON" in os.environ:
                del os.environ["STEADYTEXT_DISABLE_DAEMON"]
            os.environ["STEADYTEXT_DAEMON_HOST"] = "localhost"
            os.environ["STEADYTEXT_DAEMON_PORT"] = "5561"
            
            client = DaemonClient(host="localhost", port=5561)
            assert client.connect(), "Failed to connect to daemon"
