...
def run_speed_benchmarks(args):
    ...
    cmd = [
        ...
        "--generation-iterations", str(args.generation_iterations),
        "--embedding-iterations", str(args.embedding_iterations),
    ]
    ...
    if args.quick:
        ...
        cmd.extend([
            "--generation-iterations", "10",
            "--embedding-iterations", "10",
            "--model-loading-iterations", "1",
            "--concurrent-iterations", "2",
            "--skip-streaming",
            "--skip-cache",
            "--skip-concurrent"
        ])
...
def run_accuracy_benchmarks(args):
    ...
    cmd = [
        ...
        "--benchmarks", "simple" if args.quick else "all",
    ]
    ...
    # Output file
    accuracy_output = Path(args.output_dir) / f"accuracy_benchmark_{args.timestamp}.json"
...
Examples:
  ...
  python benchmarks/run_all_benchmarks.py --output-dir results/custom
    ...
    # Benchmark selection
    parser.add_argument('--only', choices=['speed', 'accuracy'],
                       help='Run only specific benchmark type')
