...
def run_simple_accuracy_tests(args):
    ...
    has_code_markers = any(marker in code_output for marker in ["def ", "return", ":", "(", ")"])
    ...
    results["tests"]["quality"] = {
        check[0]: check[1] for check in quality_checks
    }
...
def format_results_summary(results: dict) -> str:
    ...
    if "determinism_report" in results:
        ...
        lines.append(f"  - Avg generation time: {report.get('average_generation_time', 0):.3f}s")
...
def main():
    ...
    # Benchmark selection
    parser.add_argument('--benchmarks', choices=['standard', 'custom', 'all', 'simple'], 
                       default='all',
                       help='Which benchmarks to run (default: all)')
    
    # Task configuration
    parser.add_argument('--tasks', nargs='+', 
                       help='Specific LightEval tasks to run')
    ...
    parser.add_argument('--output-dir', type=str,
                       help='Directory for LightEval outputs')
    ...
    try:
        print(f"\n{'='*80}")
        print(f"SteadyText Accuracy Benchmarks - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"{'='*80}\n")
