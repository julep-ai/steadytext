            ...
            with self._transaction() as conn:
                for key, value in data["data"].items():
                    ...
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """, (str(key), pickled_value, frequency, adjusted_access, len(pickled_value)))
    ...
    def _evict_until_size_ok(self) -> None:
        ...
        with self._transaction() as conn:
            ...
            if keys_to_remove:
                ...
                conn.execute(f"DELETE FROM cache WHERE key IN ({placeholders})", keys_to_remove)
                
                logger.info(f"Evicted {len(keys_to_remove)} entries to free {removed_size} bytes")
            ...
            with self._transaction() as conn:
                ...
                if existing:
                    ...
                    conn.execute("""
                        ...
                        WHERE key = ?
                    """, (pickled_value, new_frequency, current_time, value_size, str(key)))
                else:
                    # Insert new entry
                    conn.execute("""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "utilization": stats[1] / self.max_size_bytes if self.max_size_bytes > 0 else 0
            }
