...
def get_config_dir() -> Path:
    ...
    if os.name == 'nt':  # Windows
        config_dir = Path(os.environ.get('APPDATA', Path.home() / 'AppData' / 'Roaming')) / 'steadytext'
    else:  # Unix-like systems
        config_dir = Path(os.environ.get('XDG_CONFIG_HOME', Path.home() / '.config')) / 'steadytext'
...
class ConfigManager:
    ...
    def set_command_defaults(self, command: str, defaults: Dict[str, Any]) -> None:
        ...
        if defaults:
            ...
            filtered_defaults = {k: v for k, v in defaults.items() 
    ...
    def merge_defaults(self, command: str, cli_params: Dict[str, Any], 
...
def merge_defaults(command: str, cli_params: Dict[str, Any], 
                  env_vars: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Merge defaults with CLI parameters and environment variables."""
