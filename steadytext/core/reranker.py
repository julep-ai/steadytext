...
def _fallback_rerank_score(query: str, document: str, seed: int = DEFAULT_SEED) -> float:
...
class DeterministicReranker:
    def __init__(self) -> None:
        ...
        self._context_window = get_optimal_context_window(model_name="qwen3-reranker-4b")
        ...
        if self.model is not None:
            try:
                ...
                if hasattr(self.model, "tokenize"):
                    ...
                    yes_tokens = self.model.tokenize("yes".encode("utf-8"), add_bos=False)
            ...
            if cache is not None:
                try:
                    ...
                    if cached is not None:
                        score = cached
                        logger.debug(f"Reranking cache hit for query='{query[:50]}...', doc='{doc[:50]}...'")
                except Exception as e:
                    logger.warning(f"Error accessing reranking cache: {e}")
            
            # If not in cache, compute score
            if score is None:
                if self.model is None or self._yes_token_id is None or self._no_token_id is None:
                    # Use fallback scoring
                    logger.debug("Using fallback reranking due to missing model or token IDs")
                    ...
                    try:
                        ...
                        if response and "choices" in response and response["choices"]:
                            ...
                            if "logprobs" in choice and choice["logprobs"]:
                                ...
                                logger.warning("No logprobs in model response, using fallback")
                                score = _fallback_rerank_score(query, doc, seed)
                        else:
                            logger.warning("Invalid model response format, using fallback")
                            score = _fallback_rerank_score(query, doc, seed)
